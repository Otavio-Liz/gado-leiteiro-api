...
def listar_producoes(banco: Session = Depends(pegar_banco)):
    producoes = banco.query(Producao).all()
    return producoes

@roteador.get("/animal/{animal_id}", response_model=List[ProducaoResposta])
def listar_producoes_por_animal(animal_id: int, banco: Session = Depends(pegar_banco)):
...
def criar_producao(producao: ProducaoCriar, banco: Session = Depends(pegar_banco)):
...
def deletar_producao(producao_id: int, banco: Session = Depends(pegar_banco)):
