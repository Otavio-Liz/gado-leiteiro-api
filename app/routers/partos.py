...
def listar_partos(banco: Session = Depends(pegar_banco), usuario: str = Depends(pegar_usuario_atual)):
    return banco.query(Parto).all()

@roteador.get("/animal/{animal_id}", response_model=List[PartoResposta])
def listar_partos_por_animal(animal_id: int, banco: Session = Depends(pegar_banco), usuario: str = Depends(pegar_usuario_atual)):
...
def criar_parto(parto: PartoCriar, banco: Session = Depends(pegar_banco), usuario: str = Depends(pegar_usuario_atual)):
...
def deletar_parto(parto_id: int, banco: Session = Depends(pegar_banco), usuario: str = Depends(pegar_usuario_atual)):
