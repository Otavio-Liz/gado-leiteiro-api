...
def listar_medicamentos(banco: Session = Depends(pegar_banco), usuario: str = Depends(pegar_usuario_atual)):
    return banco.query(Medicamento).all()

@roteador.get("/animal/{animal_id}", response_model=List[MedicamentoResposta])
def listar_medicamentos_por_animal(animal_id: int, banco: Session = Depends(pegar_banco), usuario: str = Depends(pegar_usuario_atual)):
...
def criar_medicamento(medicamento: MedicamentoCriar, banco: Session = Depends(pegar_banco), usuario: str = Depends(pegar_usuario_atual)):
...
def deletar_medicamento(medicamento_id: int, banco: Session = Depends(pegar_banco), usuario: str = Depends(pegar_usuario_atual)):
