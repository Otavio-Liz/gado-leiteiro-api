...
class ProducaoBase(BaseModel):
    ...
    @validator("motivo_descarte", always=True)
    def validar_motivo_descarte(cls, v, values):
        if values.get("status") == "descartado" and not v:
...
class ProducaoAnimalRelatorio(BaseModel):
    ...
    valor_total:            Decimal  # total_litros_aproveitados * preco_litro
